...
elementMap.has(e)||elementMap.set(e,

new Map);

const i=elementMap.get(e);

i.has(t)||0===i.size?i.set(t,

n):console.error(`Bootstrap doesn't allow more than one instance per element. Bound instance: ${
...
t)=>elementMap.has(e)&&elementMap.get(e).get(t)||null,
...
parseSelector=e=>(e&&window.CSS&&window.CSS.escape&&(e=e.replace(/#([^\s"#']+)/g,
...
n=e)=>"function"==typeof e?e.call(...t):n,

executeAfterTransition=(e,
...
t.removeEventListener(TRANSITION_END,
...
t.addEventListener(TRANSITION_END,
...
r=r.parentNode)for(const a of o)if(a===r)return hydrateObj(s,
...
if(!t.relatedTarget||t.relatedTarget!==t.delegateTarget&&!t.delegateTarget.contains(t.relatedTarget))return e.call(this,
...
const E=makeEventUid(r,

t.replace(namespaceRegex,

"")),

d=o?bootstrapDelegationHandler(e,

n,

r):bootstrapHandler(e,
...
const o=findHandler(t[n],

i,

s);

o&&(e.removeEventListener(n,
...
r]of Object.entries(s))o.includes(i)&&removeHandler(e,
...
if(_)for(const n of Object.keys(l))removeNamespacedHandlers(e,
...
i]of Object.entries(c)){

const s=n.replace(stripUidRegex,

"");

a&&!t.includes(s)||removeHandler(e,
...
t!==getTypeEvent(t)&&i&&(s=i.Event(t,
...
removeDataAttribute(e,

t){

e.removeAttribute(`data-bs-${
...
t)=>normalizeData(e.getAttribute(`data-bs-${
...
throw new Error('You have to implement the static method "NAME",
...
t=document.documentElement)=>[].concat(...Element.prototype.querySelectorAll.call(t,

e)),

findOne:(e,

t=document.documentElement)=>Element.prototype.querySelector.call(t,

e),

children:(e,

t)=>[].concat(...e.children).filter(e=>e.matches(t)),
...
e).filter(e=>!isDisabled(e)&&isVisible(e))}
...
enableDismissTrigger=(e,

t="hide")=>{

const n=`click.dismiss${
...
"AREA"].includes(this.tagName)&&n.preventDefault(),

isDisabled(this))return;

const s=SelectorEngine.getElementFromSelector(this)||this.closest(`.${
...
EVENT_CLOSE).defaultPrevented)return;
...
if(void 0===t[e]||e.startsWith("_")||"constructor"===e)throw new TypeError(`No method named "${
...
this._supportPointerEvents?(EventHandler.on(this._element,
...
this._element.classList.add("pointer-event")):(EventHandler.on(this._element,
...
EVENT_KEYDOWN$1=`keydown${

EVENT_KEY$8}

`,

EVENT_MOUSEENTER$1=`mouseenter${
...
SELECTOR_DATA_SLIDE="[data-bs-slide],
...
this._config.ride&&(this._isSliding?EventHandler.one(this._element,
...
if(this._isSliding)return void EventHandler.one(this._element,
...
"hover"===this._config.pause&&(EventHandler.on(this._element,
...
const t=SelectorEngine.findOne(".active",
...
const n=SelectorEngine.findOne(`[data-bs-slide-to="${
...
const t=Number.parseInt(e.getAttribute("data-bs-interval"),
...
s=t||getNextActiveElement(this._getItems(),
...
r=t=>EventHandler.trigger(this._element,
...
return SelectorEngine.findOne(SELECTOR_ACTIVE_ITEM,

this._element)}

_getItems(){

return SelectorEngine.find(SELECTOR_ITEM,
...
const t=Carousel.getOrCreateInstance(this,
...
void n._maybeEnableCycle()):"next"===Manipulator.getDataAttribute(this,
...
EVENT_HIDDEN$6=`hidden${

EVENT_KEY$7}

`,

EVENT_CLICK_DATA_API$4=`click${
...
SELECTOR_ACTIVES=".collapse.show,
...
EVENT_SHOW$6).defaultPrevented)return;
...
EVENT_HIDE$6).defaultPrevented)return;
...
t&&!this._isShown(t)&&this._addAriaAndCollapsedClass([e],
...
e&&this._addAriaAndCollapsedClass([t],
...
return SelectorEngine.find(e,

this._config.parent).filter(e=>!t.includes(e))}

_addAriaAndCollapsedClass(e,

t){

if(e.length)for(const n of e)n.classList.toggle("collapsed",
...
const n=Collapse.getOrCreateInstance(this,

t);

if("string"==typeof e){

if(void 0===n[e])throw new TypeError(`No method named "${
...
EVENT_CLICK_DATA_API$3=`click${

EVENT_KEY$6}

.data-api`,

EVENT_KEYDOWN_DATA_API=`keydown${

EVENT_KEY$6}

.data-api`,

EVENT_KEYUP_DATA_API=`keyup${
...
this._menu=SelectorEngine.next(this._element,

SELECTOR_MENU)[0]||SelectorEngine.prev(this._element,

SELECTOR_MENU)[0]||SelectorEngine.findOne(SELECTOR_MENU,
...
"ontouchstart"in document.documentElement&&!this._parent.closest(".navbar-nav"))for(const e of[].concat(...document.body.children))EventHandler.on(e,
...
if("ontouchstart"in document.documentElement)for(const e of[].concat(...document.body.children))EventHandler.off(e,
...
Manipulator.removeDataAttribute(this._menu,
...
if("object"==typeof(e=super._getConfig(e)).reference&&!isElement(e.reference)&&"function"!=typeof e.reference.getBoundingClientRect)throw new TypeError(`${
...
this._popper=Popper.createPopper(e,
...
return(this._inNavbar||"static"===this._config.display)&&(Manipulator.setDataAttribute(this._menu,
...
const n=SelectorEngine.find(SELECTOR_VISIBLE_ITEMS,

this._menu).filter(e=>isVisible(e));

n.length&&getNextActiveElement(n,
...
const t=Dropdown.getOrCreateInstance(this,

e);

if("string"==typeof e){

if(void 0===t[e])throw new TypeError(`No method named "${
...
ARROW_DOWN_KEY$1].includes(e.key);
...
const s=this.matches(SELECTOR_DATA_TOGGLE$3)?this:SelectorEngine.prev(this,
...
EVENT_FOCUSIN$2=`focusin${

EVENT_KEY$5}

`,

EVENT_KEYDOWN_TAB=`keydown.tab${
...
const SELECTOR_FIXED_CONTENT=".fixed-top,
...
_saveInitialAttribute(e,

t){

const n=e.style.getPropertyValue(t);

n&&Manipulator.setDataAttribute(e,

t,

n)}

_resetElementAttributes(e,
...
null!==n?(Manipulator.removeDataAttribute(e,
...
_applyManipulationCallback(e,

t){

if(isElement(e))t(e);

else for(const n of SelectorEngine.find(e,
...
this._isShown||this._isTransitioning||EventHandler.trigger(this._element,
...
EVENT_HIDE$4).defaultPrevented||(this._isShown=!1,
...
EventHandler.off(window,

".bs.modal"),

EventHandler.off(this._dialog,
...
const t=SelectorEngine.findOne(".modal-body",
...
"AREA"].includes(this.tagName)&&e.preventDefault(),
...
e.defaultPrevented||EventHandler.one(t,
...
EVENT_HIDE_PREVENTED=`hidePrevented${

EVENT_KEY$3}

`,

EVENT_HIDDEN$3=`hidden${
...
EVENT_CLICK_DATA_API$1=`click${

EVENT_KEY$3}

.data-api`,

EVENT_KEYDOWN_DISMISS=`keydown.dismiss${
...
EVENT_HIDE$3).defaultPrevented||(this._focustrap.deactivate(),
...
"static"!==this._config.backdrop?this.hide():EventHandler.trigger(this._element,
...
"Escape"===e.key&&(this._config.keyboard?this.hide():EventHandler.trigger(this._element,
...
const t=Offcanvas.getOrCreateInstance(this,
...
uriAttributes=new Set(["background",
...
const i=(new window.DOMParser).parseFromString(e,
...
for(const t of i)allowedAttribute(t,

s)||e.removeAttribute(t.nodeName)}
...
n]of Object.entries(this._config.content))this._setContent(e,
...
n]of Object.entries(e))super._typeCheckConfig({
...
const i=SelectorEngine.findOne(n,

e);

i&&((t=this._resolvePossibleFunction(t))?isElement(t)?this._putElementInTemplate(getElement(t),

i):this._config.html?i.innerHTML=this._maybeSanitize(t):i.textContent=t:i.remove())}

_maybeSanitize(e){

return this._config.sanitize?sanitizeHtml(e,

this._config.allowList,

this._config.sanitizeFn):e}
...
_putElementInTemplate(e,

t){

if(this._config.html)return t.innerHTML="",
...
EventHandler.off(this._element.closest(".modal"),
...
"ontouchstart"in document.documentElement)for(const e of[].concat(...document.body.children))EventHandler.on(e,
...
if(this._isShown()&&!EventHandler.trigger(this._element,

this.constructor.eventName("hide")).defaultPrevented){

if(this._getTipElement().classList.remove("show"),

"ontouchstart"in document.documentElement)for(const e of[].concat(...document.body.children))EventHandler.off(e,
...
const t=execute(this._config.placement,
...
return Popper.createPopper(this._element,
...
for(const t of e)if("click"===t)EventHandler.on(this._element,
...
t._activeTrigger.click=!(t._isShown()&&t._activeTrigger.click),
...
e&&(this._element.getAttribute("aria-label")||this._element.textContent.trim()||this._element.setAttribute("aria-label",
...
n]of Object.entries(this._config))this.constructor.Default[t]!==n&&(e[t]=n);
...
const t=Tooltip.getOrCreateInstance(this,
...
const t=Popover.getOrCreateInstance(this,
...
px 0px -30%`:e.rootMargin,

"string"==typeof e.threshold&&(e.threshold=e.threshold.split(",
...
this._config.smoothScroll&&(EventHandler.off(this._config.target,
...
const t=e=>this._targetLinks.get(`#${
...
const e=SelectorEngine.find("[href]",
...
isVisible(e)&&(this._targetLinks.set(decodeURI(t.hash),

t),

this._observableSections.set(t.hash,
...
if(e.classList.contains("dropdown-item"))SelectorEngine.findOne(".dropdown-toggle",

e.closest(".dropdown")).classList.add("active");

else for(const t of SelectorEngine.parents(e,

".nav,

 .list-group"))for(const e of SelectorEngine.prev(t,

SELECTOR_LINK_ITEMS))e.classList.add("active")}

_clearActiveClass(e){

e.classList.remove("active");

const t=SelectorEngine.find("[href].active",
...
const t=ScrollSpy.getOrCreateInstance(this,
...
SELECTOR_TAB_PANEL='.list-group,

 .nav,

 [role="tablist"]',

SELECTOR_OUTER=".nav-item,

 .list-group-item",

SELECTOR_INNER='.nav-link:not(.dropdown-toggle),

 .list-group-item:not(.dropdown-toggle),

 [role="tab"]:not(.dropdown-toggle)',

SELECTOR_DATA_TOGGLE='[data-bs-toggle="tab"],
...
SELECTOR_DATA_TOGGLE_ACTIVE='.active[data-bs-toggle="tab"],

 .active[data-bs-toggle="pill"],

 .active[data-bs-toggle="list"]';
...
).defaultPrevented||n&&n.defaultPrevented||(this._deactivate(t,
...
END_KEY].includes(e.key))n=t[e.key===HOME_KEY?0:t.length-1];

else{

const i=[ARROW_RIGHT_KEY,

ARROW_DOWN_KEY].includes(e.key);

n=getNextActiveElement(t,
...
return SelectorEngine.find(SELECTOR_INNER_ELEM,
...
n!==e&&this._setAttributeIfNotExists(n,

"role",

"presentation"),

t||e.setAttribute("tabindex",
...
t&&(this._setAttributeIfNotExists(t,

"role",

"tabpanel"),

e.id&&this._setAttributeIfNotExists(t,
...
const s=SelectorEngine.findOne(e,

n);

s&&s.classList.toggle(i,
...
return e.matches(SELECTOR_INNER_ELEM)?e:SelectorEngine.findOne(SELECTOR_INNER_ELEM,
...
EVENT_SHOW).defaultPrevented||(this._clearTimeout(),
...
EVENT_HIDE).defaultPrevented||(this._element.classList.add("showing"),
...
case"mouseover":case"mouseout":this._hasMouseInteraction=t;
